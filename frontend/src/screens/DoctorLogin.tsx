...
import { Button, Input, PasswordInput, Checkbox, Divider, AlertBanner } from "@/components/ui";
...
export default function DoctorLogin({ onNavigate }: DoctorLoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email) errors.email = "Work email or Staff ID is required";
    ...
    onNavigate("auth-success", { role: "doctor" });
  };
    ...
    <div className="min-h-full flex flex-col" style={{ background: "#F6F6F7" }}>
      ...
      <main className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md animate-fade-up stagger-1">
          ...
          <p className="text-sm text-[#71717A] mb-7">Sign in to continue to your clinical workspace.</p>
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => onNavigate("forgot-password", { role: "doctor" })}
          ...
          <div className="flex flex-col gap-2 mt-1">
            <p className="text-sm text-[#71717A] text-center">
              New to Vaidya?{" "}
              <button
                onClick={() => onNavigate("doctor-request")}
